...
var possibleStandardNames = (_possibleStandardName = {}, _defineProperty(_possibleStandardName, TYPES.BITMAP_TEXT, bitmapTextStandardNames), _defineProperty(_possibleStandardName, TYPES.CONTAINER, containerStandardNames), _defineProperty(_possibleStandardName, TYPES.GRAPHICS, graphicsStandardNames), _defineProperty(_possibleStandardName, TYPES.PARTICLE_CONTAINER, particleContainerStandardNames), _defineProperty(_possibleStandardName, TYPES.SPRITE, spriteStandardNames), _defineProperty(_possibleStandardName, TYPES.TEXT, textStandardNames), _defineProperty(_possibleStandardName, TYPES.TILING_SPRITE, tilingSpriteStandardNames), _possibleStandardName);
...
var StrictMode = 1; // Would be better if this was just exported from react-reconciler
...
var BOOLEAN = 2; // An attribute that must be numeric or parse as a numeric.
// When falsy, it should be removed.

var NUMERIC = 3; // An attribute that must be positive numeric or parse as a positive numeric.
// When falsy, it should be removed.

var POSITIVE_NUMERIC = 4; // An attribute that must be vector or parse as a vector.
// When falsy, it should be removed.

var VECTOR = 5; // An attribute that must be function
// When falsy, it should be removed.
...
var properties = {};
var customProperties = {}; // These props are reserved by React. They shouldn't be written to the DOM.
...
[// pixi.js < 7.0
"added", "click", "mousedown", "mousemove", "mouseout", "mouseover", "mouseup", "mouseupoutside", "pointercancel", "pointerdown", "pointermove", "pointerout", "pointerover", "pointertap", "pointerup", "pointerupoutside", "removed", "rightclick", "rightdown", "rightup", "rightupoutside", "tap", "touchcancel", "touchend", "touchendoutside", "touchmove", "touchstart", // pixi.js >= 7.1
...

{
  ...
  exports.CustomPIXIProperty = function CustomPIXIProperty(maybeComponentType, propertyName, validator) {
    var lowerCasedPropertyName = propertyName.toLowerCase(); // Define custom property for all component types if none was provided
    ...
    componentTypeList.forEach(function (componentType) {
      ...
      possibleStandardNames[componentType][lowerCasedPropertyName] = propertyName; // inject custom property info
...
var _defaultProps;
var CHILDREN = "children"; // http://pixijs.download/release/docs/PIXI.DisplayObject.html
...
var defaultProps$1 = (_defaultProps = {}, _defineProperty(_defaultProps, TYPES.BITMAP_TEXT, bitmapTextDefaultProps), _defineProperty(_defaultProps, TYPES.CONTAINER, containerDefaultProps), _defineProperty(_defaultProps, TYPES.GRAPHICS, graphicsDefaultProps), _defineProperty(_defaultProps, TYPES.PARTICLE_CONTAINER, particleContainerDefaultProps), _defineProperty(_defaultProps, TYPES.SPRITE, spriteDefaultProps), _defineProperty(_defaultProps, TYPES.TEXT, textDefaultProps), _defineProperty(_defaultProps, TYPES.TILING_SPRITE, tilingSpriteDefaultProps), _defaultProps);
...
function appendChild(parentInstance, child) {
  if (parentInstance == null) return; // TODO do we need to remove the child first if it's already added?
...
function preparePortalMount(containerInfo) {// Noop
}
...
function commitTextUpdate(textInstance, prevText, nextText) {// Noop
}
...
function cleanupStage(app) {
  ...
  var removeView = false; // Unmount stage tree

  unmount$1(app.stage); // Give components a chance to finish unmounting before destroying PIXI.Application
...
function useStageRenderer(props, appRef, canvasRef) {
  ...
  React.useLayoutEffect(function () {
    var app = props.app,
        options = props.options; // Return PIXI.Application if it was provided in props

    if (app != null) {
      invariant_1(app instanceof PIXI__namespace.Application, "Provided `app` has to be an instance of PIXI.Application");
      appRef.current = app;
      renderStage(appRef.current, props); // Not destroying provided PIXI.Application when unmounting

      return;
    }

    var view = canvasRef.current; // Create new PIXI.Application
    // Canvas passed in options as `view` will be used if provided

    appRef.current = createPixiApplication(_objectSpread2({
      view: view
    }, options));
    renderStage(appRef.current, props); // Cleanup current PIXI.Application when unmounting
...
function useStageRerenderer(props, appRef, canvasRef) {
  var prevProps = usePreviousProps(props); // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  React.useLayoutEffect(function () {
    ...
    var options = props.options,
        ...
        var otherOptions = _objectWithoutProperties(_props$options, ["height", "width"]);

    var _prevProps$options = prevProps.options;
        _prevProps$options.height;
        _prevProps$options.width;
        var prevOtherOptions = _objectWithoutProperties(_prevProps$options, ["height", "width"]);

    var view = canvasRef.current; // We need to create new PIXI.Application when options other than dimensions
    // are changed because some renderer settings are immutable.

    if (!shallowEqual_1(otherOptions, prevOtherOptions)) {
      // Destroy PIXI.Application
      cleanupStage(appRef.current, STAGE_OPTIONS_RECREATE); // Create new PIXI.Application
      // Canvas passed in options as `view` will be used if provided

      appRef.current = createPixiApplication(_objectSpread2({
        view: view
      }, options)); // Set initial properties

      renderStage(appRef.current, props);
    } else {
      // Update stage tree
      rerenderStage(appRef.current, prevProps, props); // Update canvas and renderer dimestions
...
function createStageFunction() {
  var Stage = /*#__PURE__*/React.forwardRef(function Stage(props, ref) {
    var app = props.app,
        options = props.options; // Store PIXI.Application instance

    var appRef = React.useRef(); // Store canvas if it was rendered
    ...
    useStageRenderer(props, appRef, canvasRef); // Do not render anything if PIXI.Application was provided in props
...
function createStageClass() {
  var Stage = /*#__PURE__*/function (_React$Component) {
    ...
    function Stage(props) {
      ...
      _this = _super.call(this, props); // Store PIXI.Application instance

      _this._app = createRef(); // Store canvas if it was rendered
    ...
    _createClass(Stage, [{
      ...
      value: function componentDidMount() {
        ...
        this.renderStage(app, this.props); // Store app instance
      ...
      value: function componentDidUpdate(prevProps) {
        ...
        this.rerenderStage(app, this.props, prevProps); // Update stored app instance
      ...
      value: function componentWillUnmount() {
        var app = this.props.app; // Destroy PIXI.Application if it was not provided in props
      ...
      value: function render() {
        var _this$props = this.props,
            app = _this$props.app,
            options = _this$props.options; // Do not render anything if PIXI.Application was provided in props
      ...
      value: function getPixiApplication(props, prevProps) {
        ...
        var view = this._canvas.current; // Render stage for the first time
        ...
        var _prevProps$options = prevProps.options;
            _prevProps$options.height;
            _prevProps$options.width;
            var prevOtherOptions = _objectWithoutProperties(_prevProps$options, ["height", "width"]); // We need to create new Application when options other than dimensions
        // are changed because some of the renderer settings are immutable


        if (!shallowEqual_1(otherOptions, prevOtherOptions)) {
          // Destroy PIXI.Application
          cleanupStage(this._app.current, STAGE_OPTIONS_RECREATE); // Create new PIXI.Application
          // Canvas passed in options as `view` will be used if provided
      ...
      value: function renderStage$1(app, props) {
        ...
        if (app === this._app.current) return; // Set initial properties
      ...
      value: function rerenderStage$1(app, props, prevProps) {
        ...
        if (app !== this._app.current) return; // Update stage tree

        rerenderStage(app, prevProps, props, this); // Update canvas and renderer dimestions
